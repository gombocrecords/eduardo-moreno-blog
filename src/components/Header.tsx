        ...
        <div className="flex items-center justify-between py-4">
          <Link href="/" className="group">
            ...
            <h1 className="text-xl sm:text-2xl font-serif font-bold text-navy-600 leading-tight group-hover:text-teal-500 transition-colors">
              Movilidad, Logística e Infraestructura
            </h1>
          </Link>
            ...
            {NAV.map(({ label, href }) => {
              ...
              return (
                <Link key={href} href={href}
                  className={`px-3 py-2 rounded-md text-sm font-semibold transition-colors ${active ? 'bg-navy-600 text-white' : 'text-gray-700 hover:bg-navy-50 hover:text-navy-600'}`}>
                  {label}
                </Link>
              )
            })}
            ...
            {NAV.map(({ label, href }) => {
              ...
              return (
                <Link key={href} href={href} onClick={() => setOpen(false)}
                  className={`block px-4 py-3 rounded-md text-base font-semibold transition-colors ${active ? 'bg-navy-600 text-white' : 'text-gray-700 hover:bg-navy-50 hover:text-navy-600'}`}>
                  {label}
                </Link>
              )
            })}
